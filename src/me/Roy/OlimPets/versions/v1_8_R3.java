...
public class v1_8_R3 implements Packets {
    ...
    public void ponerFakeArmor(Player p, Integer entityid) {
        Pet pet = pets.get(p);
        PacketPlayOutEntityEquipment packet = new PacketPlayOutEntityEquipment(entityid, 4, CraftItemStack.asNMSCopy(pet.getHelmet()));
        ((CraftPlayer) p).getHandle().playerConnection.sendPacket(packet);
        
        packet = new PacketPlayOutEntityEquipment(entityid, 3, CraftItemStack.asNMSCopy(pet.getChestplate()));
        ((CraftPlayer) p).getHandle().playerConnection.sendPacket(packet);
        
        packet = new PacketPlayOutEntityEquipment(entityid, 2, CraftItemStack.asNMSCopy(pet.getLeggings()));
        ((CraftPlayer) p).getHandle().playerConnection.sendPacket(packet);
        
        packet = new PacketPlayOutEntityEquipment(entityid, 1, CraftItemStack.asNMSCopy(pet.getBoots()));
        ((CraftPlayer) p).getHandle().playerConnection.sendPacket(packet);
        
        packet = new PacketPlayOutEntityEquipment(entityid, 0, CraftItemStack.asNMSCopy(pet.getMano()));
        ((CraftPlayer) p).getHandle().playerConnection.sendPacket(packet);
    }
